...
export class KhachThueComponent implements OnInit {
  ...
  loadKhachThues() {
    this.apiService.getKhachThues(this.searchTerm).subscribe({
      next: (data) => this.khachThues = data,
      error: (err) => this.toastService.error('Lỗi tải dữ liệu')
    });
  }
  ...
  saveChuyenPhong() {
    this.apiService.chuyenPhong(this.chuyenPhongData.khachThueId, {
      phongTroMoiId: parseInt(this.chuyenPhongData.phongTroMoiId)
    }).subscribe({
      next: () => {
        this.loadKhachThues();
        this.loadPhongTros();
